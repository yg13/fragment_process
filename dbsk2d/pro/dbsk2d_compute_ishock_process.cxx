  ...
  for (unsigned int b = 0 ; b < vsol_list.size() ; b++ )
  {
    ...
    else if( vsol_list[b]->cast_to_curve())
    {
      ...
      if( vsol_list[b]->cast_to_curve()->cast_to_line() )
      {
        ...
        if (bnoise_contour) {
          vsol_polyline_2d_sptr polyline = add_noise_to_contour(
...
fit_lines_to_contour(vsol_polyline_2d_sptr poly, double rms) {
  if (rms > 0) {  // vgl_fit
    ...
    vcl_vector<vsol_point_2d_sptr > new_pts;
	  vsol_point_2d_sptr pt1 = new vsol_point_2d(segs[0].point1().x(),segs[0].point1().y());
	  vsol_point_2d_sptr pt2 = new vsol_point_2d(segs[0].point2().x(),segs[0].point2().y());
    new_pts.push_back(pt1);
    new_pts.push_back(pt2);
